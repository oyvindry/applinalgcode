function x = DWTKernelOrtho(x, filters, bd_mode, dual)
    ...
    %          liftingfactortho()
    % bd_mode: Boundary extension mode - the following modes are supported:     
    ...
    if dual
        ...
        for stepnr = size(filters.lambdas,1):(-2):2
            ...
            x = liftingstepeven(filters.lambdas(stepnr-1,2), filters.lambdas(stepnr-1,1), x, bd_mode);
        end
  
        if stepnr == 3
            x = liftingstepodd(filters.lambdas(1,2), filters.lambdas(1,1), x, bd_mode);
        end
    else
        x(1:2:N, :) = x(1:2:N, :)*filters.alpha;
        x(2:2:N, :) = x(2:2:N, :)*filters.beta;
        for stepnr = size(filters.lambdas,1):(-2):2
            ...
            x = liftingstepodd(-filters.lambdas(stepnr-1,1), -filters.lambdas(stepnr-1,2), x, bd_mode);
        end
  
        if stepnr == 3
            x = liftingstepeven(-filters.lambdas(1,1), -filters.lambdas(1,2), x, bd_mode);
        end
    end
    if bd_mode >= 2
